  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        >
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{patient.streak} days</p>
            </div>
          </div>
        </motion.div>
      ...
      <motion.div 
        ...
        className="bg-white p-6 rounded-xl shadow-md mb-8"
      >
        <h2 className="text-xl font-semibold mb-6">Assigned Exercises</h2>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {patient.assignedExercises.map(exercise => (
                  <tr key={exercise.id}>
                    <td className="px-4 py-3 text-sm font-medium">{exercise.title}</td>
                    ...
                    <td className="px-4 py-3 text-sm">{exercise.sets} × {exercise.reps}</td>
                    <td className="px-4 py-3 text-sm">{exercise.duration}s</td>
                    <td className="px-4 py-3 text-sm">{exercise.completed} times</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {patient.progress.slice().reverse().map((progress, index) => (
                  <tr key={index}>
                    <td className="px-4 py-3 text-sm">{progress.date}</td>
                    <td className="px-4 py-3 text-sm">{progress.completed}</td>
                    <td className="px-4 py-3 text-sm">{progress.timeSpent} min</td>
                    <td className="px-4 py-3 text-sm">{progress.accuracy}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
